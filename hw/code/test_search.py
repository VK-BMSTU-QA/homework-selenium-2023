...
class TestSearch(BasePage):
    ...
    def test_check_group_category(self):
        ...
        serials_field = self.find_group((By.CLASS_NAME, self.CLASS_NAME_CATEGORY_TITLE))[self.CLASS_NAME_SEARCH_GROUP_SERAILS].text
        if serials_field != self.SEARCH_GROUP_SERAILS:
            raise Exception("stings does not equal", serials_field, self.SEARCH_GROUP_SERAILS)

        persons_field = self.find_group((By.CLASS_NAME, self.CLASS_NAME_CATEGORY_TITLE))[self.CLASS_NAME_SEARCH_GROUP_PERSONS].text
    ...
    def test_correct_results(self):
        ...
        self.find((By.CLASS_NAME, self.CLASS_NAME_FIRST_FOUNDED_FILM)).click()
        film_title = self.find((By.CLASS_NAME, self.CLASS_NAME_TITLE_ON_FILM_PAGE)).text
